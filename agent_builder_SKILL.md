...
prompt _source_ when they sit in mutually exclusive `{% if %}` / `{% elif %}` / `{% else %}`
...
11. **One say block per language** — no branch of a prompt contains two `<say>` blocks with the same `lang`. Multi-sentence lines live inside a single block. (Same-language blocks in _different_ Jinja branches are fine, since only one branch renders.)
